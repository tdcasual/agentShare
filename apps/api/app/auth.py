...
def resolve_runtime_principal(api_key: str, session: Session) -> RuntimePrincipal | None:
    ...
    return RuntimePrincipal(
        ...
        subject_type=access_token.subject_type,
        subject_id=access_token.subject_id,
...
def ensure_task_type_allowed(agent: RuntimePrincipal, task_type: str) -> None:
...
def ensure_capability_allowed(agent: RuntimePrincipal, capability_id: str) -> None:
...
def ensure_runtime_scope_allowed(agent: RuntimePrincipal) -> None:
    if agent.scopes and "runtime" not in set(agent.scopes):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access token lacks runtime scope",
        )
