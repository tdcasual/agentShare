...
class TaskTargetRepository:
    ...
    def list_assigned(self, token_id: str) -> list[TaskTargetModel]:
        return list(
            self.session.query(TaskTargetModel)
            .filter(TaskTargetModel.target_token_id == token_id)
            .all()
        )

    def find_by_task_and_token(self, task_id: str, token_id: str) -> TaskTargetModel | None:
        return (
            self.session.query(TaskTargetModel)
            .filter(TaskTargetModel.task_id == task_id)
            .filter(TaskTargetModel.target_token_id == token_id)
