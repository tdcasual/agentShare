...
export function OperationsFeed({
  ...
  return (
    <Card className="dark:bg-[var(--kw-dark-surface)]/90 bg-[var(--kw-surface)]/90 space-y-5 border border-[var(--kw-border)] dark:border-[var(--kw-dark-border)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-[var(--kw-text)] sm:text-xl">
            {t('spaces.sections.operationsSpaceTitle')}
          </h2>
          <p className="mt-1 text-sm text-[var(--kw-text-muted)]">
            {t('spaces.sections.operationsSpaceDescription')}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {activeAgents.map((agent) => (
          <Button
            ...
            aria-label={t('spaces.sections.showActivityFor', { id: agent.id })}
          >
            {t('spaces.sections.agentLabel', { name: agent.name })}
          </Button>
        ))}
      </div>
