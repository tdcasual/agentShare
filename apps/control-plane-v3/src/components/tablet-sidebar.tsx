...
export function TabletBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-16 border-t border-[var(--kw-border)] bg-[var(--kw-surface)] md:hidden dark:border-[var(--kw-dark-border)] dark:bg-[var(--kw-dark-surface)]">
      <div className="flex h-full items-center justify-around px-4">
        {navItems.slice(0, 4).map((item) => (
          <Link
            ...
            href={item.href}
            className={cn(
              'flex flex-col items-center gap-1 rounded-lg p-2 transition-colors focus-visible:ring-2 focus-visible:ring-[var(--kw-primary-400)]',
              pathname === item.href
          ...
          >
            <span className="relative">
              {item.icon}
              {item.badge && (
                <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[var(--kw-primary-500)] text-[10px] font-bold text-white">
                  {item.badge}
                </span>
              )}
            </span>
