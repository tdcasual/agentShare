...
BOOTSTRAP_AGENT_KEY="${BOOTSTRAP_AGENT_KEY:-changeme-bootstrap-key}"
...
Backend command
cd ${API_DIR}
APP_ENV=development DATABASE_URL=${DATABASE_URL} DEMO_SEED_ENABLED=${DEMO_SEED_ENABLED} BOOTSTRAP_AGENT_KEY=${BOOTSTRAP_AGENT_KEY} MANAGEMENT_SESSION_SECRET=${MANAGEMENT_SESSION_SECRET} ${UV_BIN} run uvicorn app.main:app --host ${HOST} --port ${API_PORT}
