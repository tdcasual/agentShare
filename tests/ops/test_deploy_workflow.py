...
def test_deploy_workflow_validates_remote_stack_and_runs_smoke_checks() -> None:
    ...
    # The remote script exports the small set of values needed by ops scripts
    # without evaluating the dotenv file as shell code.
    assert "read_env_value()" in workflow
    assert 'export PUBLIC_HOST="$(read_env_value PUBLIC_HOST)"' in workflow
    assert workflow.index("read_env_value()") < workflow.index("./scripts/ops/smoke-test.sh")
    assert ". ./.env.production" not in workflow
    assert ". ./.release.env" not in workflow
    assert "umask 077" in workflow
    assert "chmod 600 .env.production" in workflow
